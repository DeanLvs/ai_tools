...
from diffusers import StableDiffusionPipeline, UNet2DConditionModel, AutoencoderKL, PNDMScheduler, DDIMScheduler
...
class ImageProcessor(metaclass=SingletonMeta):
    def __init__(self, model_path="stable-diffusion-inpainting", checkpoint_paths=None, lora_path=None,sampler="DPM++ SDE Karras", cfg_scale=7.5):
    ...
    def load_lora_weights_se(self, lora_path):
        try:
            # 加载本地 Lora 加速器权重
            lora_weights = load_safetensors(lora_path)
            lora_keys = self._transform_keys_with_keymap(lora_weights)

            # 获取模型的状态字典
            model_keys = self.pipe.unet.state_dict().keys()
            lora_keys_set = set(lora_keys.keys())

            # 打印模型能够接收的键
            print("Model can accept the following keys:")
            for key in model_keys:
                print(key)

            # 打印 Lora 文件中实际的键
            print("\nKeys in Lora file:")
            for key in lora_keys_set:
                print(key)

            # 加载 Lora 加速器权重
            lora_load_info = self.pipe.unet.load_state_dict(lora_keys, strict=False)
            total_lora_keys = len(model_keys)
            loaded_lora_keys = total_lora_keys - len(lora_load_info.missing_keys)

            print(f"\nLora {lora_path} Total keys: {total_lora_keys}, Loaded keys: {loaded_lora_keys}")
            if lora_load_info.missing_keys:
                print("Missing keys during Lora load:", lora_load_info.missing_keys)
            if lora_load_info.unexpected_keys:
                print("Unexpected keys during Lora load:", lora_load_info.unexpected_keys)
        except Exception as ex:
            print(f"Error loading Lora weights from {lora_path}: {ex}")
    ...
    def load_lora_weights(self, lora_path=None):
        self.pipe.load_lora_weights(lora_path)
        lora_load_state_dict,network_alphas = self.pipe.lora_state_dict(lora_path)
    ...
    def generate_image(self, prompt, reverse_prompt, init_image, mask_image):
        result = self.pipe(prompt=prompt, reverse_prompt=reverse_prompt, image=init_image, mask_image=mask_image,guidance_scale=self.cfg_scale, guidance_rescale=1).images[0]
        return result

if __name__ == '__main__':
    # 直接使用 Hugging Face Hub 上的路径
    lora_path = "/mnt/sessd/civitai-downloader/Hyper-SD15-12steps-CFG-lora.safetensors"  # 替换为实际路径 # 或 "https://huggingface.co/ByteDance/Hyper-SD/blob/main/Hyper-SD15-12steps-CFG-lora.safetensors"
    pipeline = ImageProcessor(checkpoint_paths=["/mnt/sessd/civitai-downloader/pornmasterPro_v8-inpainting-b.safetensors"], lora_path=lora_path,
                              sampler="DPM++ SDE Karras",  cfg_scale=7.5)
    torch.manual_seed(78)
    # 使用示例
    init_image_path = "/mnt/sessd/ai_tools/static/uploads/frame_0196.png"
    mask_image_path = "/mnt/sessd/ai_tools/static/uploads/big_mask_frame_0196.png"

    # 加载图像
    init_image = Image.open(init_image_path).convert("RGB")
    mask_image = Image.open(mask_image_path).convert("L")
    original_width, original_height = init_image.size
    prompt = "R/bigasses,R/pussy,R/boobs,naked,really detailed skin,reddit,match the original pose,match the original image angle,Match the light of the original image,natural body proportions"
    result_image = pipeline.generate_image(prompt, "deformed,bad anatomy, mutated,long neck,disconnected limbs,clothing,apparel,wearables", init_image, mask_image)
