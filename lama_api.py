...
import sys
import traceback
sys.path.append("/nvme0n1-disk/ai_tools/lama")
import io
import torch
import yaml
...
class LamaModelSingleton:
    _instance = None
    _lock = threading.Lock()  # A lock object to ensure thread-safe singleton access

    def __new__(cls, model_dir='/nvme0n1-disk/ai_tools/big-lama', device='cuda'):
...
def load_image_l(mask_img, return_orig=False):
    ...
    if return_orig:
        ...
        return out_img
def inpaint_image(model, image_path, maskImag, device='cuda', refine=False, iterations=1):
    """
    使用 LaMa 模型进行图像修复
    :param model: LaMa 模型
...
async def inpaint(
    ...
    try:
        ...
        img_io.seek(0)
        torch.cuda.empty_cache()
        re_it = StreamingResponse(img_io, media_type="image/png")
