...
def genIt(processor, prompt, prompt_2, reverse_prompt, reverse_prompt_2, genImage, big_mask,
          ...
          controlnet_conditioning_scale=[1.0, 0.8, 1.0]):
    try:
        ...
        if len(processor.controlnet_list) == 1:
            to_control_image = control_image[0]
            to_controlnet_conditioning_scale = controlnet_conditioning_scale[0]
        else:
            to_control_image = control_image
            to_controlnet_conditioning_scale = controlnet_conditioning_scale
        with torch.no_grad():
            resultTemp = processor.pipe(
                ...
                controlnet_conditioning_scale=to_controlnet_conditioning_scale,
...
def handle_image_processing_b(data):
    ...
    try:
        if mask_future is not None:
            mask_clear_finally, temp_humanMask, control_image_return, normal_map_img_return = mask_future.result()
        ...
        orgResult = genIt(processor, prompt, prompt_2, reverse_prompt, reverse_prompt_2, orgImage, big_mask, num_inference_steps, guidance_scale,
                          create_callback('1/4'), strength, [control_image, normal_map_img], [re_p, ga_b])
        ...
        org_fix_Result = genIt(processor, prompt, prompt_2, reverse_prompt, reverse_prompt_2, clear_org_i, big_mask, num_inference_steps, guidance_scale,
                          create_callback('2/4'), strength, [control_image, normal_map_img], [re_p, ga_b])
        ...
        if isinstance(genImage, Image.Image) and genImage is not None:
            result = genIt(processor, prompt, prompt_2, reverse_prompt, reverse_prompt_2,
                           ...
                           create_callback('3/4'), strength, [control_image, normal_map_img], [re_p, ga_b])
            result = result.resize((original_width, original_height), Image.LANCZOS)
        ...
        if next_genImage:
            result_next = genIt(processor, prompt, prompt_2, reverse_prompt, reverse_prompt_2,
                                ...
                                create_callback('4/4'), strength, [control_image, normal_map_img], [re_p, ga_b])
