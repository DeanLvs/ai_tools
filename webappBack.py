...
def re_auto_mask(filename, re_p=2, re_b=1, ha_p=2, ga_b=1):
...
def save_mask(fix_mask, filename):
    kernel = np.ones((3, 3), np.uint8)
    fix_mask_dilated = cv2.dilate(fix_mask, kernel, iterations=12)

    big_fix_path = os.path.join(app.config['UPLOAD_FOLDER'], filename)
...
def re_auto_mask_with_out_head(filename, re_p=2, re_b=0, ha_p=2, ga_b=0):
    print(f"  re_p: {re_p} re_b: {re_b} or ha_p: {ha_p}  or ga_b: {ga_b} ")
    file_path = os.path.join(app.config['UPLOAD_FOLDER'], filename)

    # 生成两个掩码
    def generate_close_mask():
        mask_generator = MaskGenerator()
        image_pil = Image.open(file_path).convert("RGB")
        temp_close_mask,temp_back_mask = mask_generator.generate_mask(image_pil)
        return temp_close_mask, convert_to_ndarray(temp_close_mask)

    # def generate_human_mask():
    #     temp_human_mask = handlerMask(file_path)
    #     return temp_human_mask, convert_to_ndarray(temp_human_mask)

    with ThreadPoolExecutor() as executor:
        # 提交任务
        future_close_mask = executor.submit(generate_close_mask)
        # future_human_mask = executor.submit(generate_human_mask)

        # 等待结果
        close_mask, densepose_mask_ndarray = future_close_mask.result()
        # human_mask, human_mask_ndarray = future_human_mask.result()

    # 合并掩码
    mask = densepose_mask_ndarray;
    # all_mask = merge_masks(human_mask_ndarray, densepose_mask_ndarray)
    ...
    def process_replace_mask():
        ...
        replace_mask.save(replace_mask_path)
        # 转换掩码为 ndarrays
        return convert_to_ndarray(replace_mask)
    ...
    def process_big_mask():
        ...
        big_mask_path = os.path.join(app.config['UPLOAD_FOLDER'], 'big_mask_' + filename)
        big_mask.save(big_mask_path)
        return convert_to_ndarray(big_mask)
    ...
    # 取反
    # fix_mask = subtract_masks(big_mask_f, replace_mask_f)
    # 增加 fix_mask 周围的像素

    # save_mask(fix_mask, 'fix_mask_' + filename)

    # save_mask(all_mask, 'all_remove_mask_' + filename)
...
def handle_image_processing(data):
    ...
    all_remove_mask_filename = 'all_remove_mask_' + filename
    fix_mask_filename = 'fix_mask_' + filename
    ...
    fix_mask_path = os.path.join(app.config['UPLOAD_FOLDER'], fix_mask_filename)
    all_remove_mask_path = os.path.join(app.config['UPLOAD_FOLDER'], all_remove_mask_filename)
    ...
    if re_mask != 'F' or not os.path.exists(mask_path) or not os.path.exists(big_mask_path):
        ...
        try:
            ...
            cur_res, success_is = model_instance.predict(file_path, big_mask_path)
            if success_is:
    ...
    try:
        ...
        # 使用掩码反转图像和原始图像混合生成的图像，以保留掩码外的区域
        ...
        cv2.imwrite(processed_r_file_path, result)
        emit('processing_done', {'processed_image_url': f'/uploads/{processed_r_file_path}'}, broadcast=True, to=room_id)

        # 对掩码进行高斯模糊处理
        blurred_mask = mask.filter(ImageFilter.GaussianBlur(radius=8))
        blended_result = Image.composite(result, image, blurred_mask)

        blended_result.save(processed_file_path, quality=70)
        ...
        if had_suc:
            ...
            blended_result_fixed.save(f_f_processed_file_path, quality=70)
            processed_results[unique_key] = processed_filename
