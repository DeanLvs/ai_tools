...
import torch
import gc
from diffusers import StableDiffusionXLInpaintPipeline, UNet2DConditionModel, AutoencoderKL, EulerDiscreteScheduler, EulerAncestralDiscreteScheduler, DPMSolverMultistepScheduler
...
class CustomInpaintPipeline:
    ...
    def __init__(self, model_path="stable-diffusion-xl-1.0-inpainting-0.1"):
        if not hasattr(self, "_initialized"):
            ...
            # self.load_lora_weights(lora_path="/mnt/sessd/civitai-downloader/lora/biggunsxl_v11.safetensors", lora_scale=0.6, adapter_name="bigsxiiiixxl")
            # self.load_lora_weights(lora_path="/mnt/sessd/civitai-downloader/lora/Breasts-05.safetensors",
            #                        lora_scale=0.6, adapter_name="bigsxlxxsd")
            # self.load_lora_weights(lora_path="/mnt/sessd/civitai-downloader/lora/Nipples XL - All in One.safetensors", lora_scale=0.2, adapter_name="nudifyitw")
            # self.load_lora_weights(lora_path="/mnt/sessd/civitai-downloader/lora/NudeXL.safetensors", lora_scale=0.3, adapter_name="bigsxlxxyyaaa")
    ...
    def generate_image(self, prompt, reverse_prompt, init_image, mask_image, num_inference_steps = 80, seed = 66, guidance_scale = 7.5, strength=0.8):
        torch.manual_seed(seed)
        result = self.pipe(prompt=prompt, reverse_prompt=reverse_prompt, image=init_image, mask_image=mask_image, num_inference_steps=num_inference_steps, guidance_scale=guidance_scale, strength=strength).images[0]
        # 释放 GPU 内存
...
if __name__ == '__main__':
    ...
    prompt = "remove clothes, nude, big asses,big boobs"
    result_image = pipeline.generate_image(prompt, "deformed,bad anatomy, mutated,long neck,disconnected limbs,unnaturally thin waist,extremely small/waist", init_image, mask_image)
    # 高分辨率调整
