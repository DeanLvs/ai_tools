...
function autoRecognize() {
    ...
    const prompt = document.getElementById('prompt_cof').value;
    const reverse_prompt = document.getElementById('reverse_prompt_cof').value;
    const prompt_2 = document.getElementById('prompt_2_cof').value;
    const reverse_prompt_2 = document.getElementById('reverse_prompt_2_cof').value;
    ...
    const room_id = checkUUID();
    fetch('/api/generate_mask', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
        },
        body: JSON.stringify({ room_id, prompt, reverse_prompt, prompt_2, reverse_prompt_2, num_inference_steps, guidance_scale, seed, strength, filename, points: maskPoints }),
    })
    .then(response => response.json())
    .then(data => {
        hideOverlay();
