...
class XFluxPipeline:
    def __init__(self, model_type, device, offload: bool = False, cust_pat='', cust_lb=['ae', 'tf'], debug=False):
        ...
        if debug:
            self.ae = load_ae_debug_ci(model_type, ci_name=cust_pat, device="cpu" if offload else self.device)
        else:
            if cust_pat != '' and 'ae' in cust_lb:
                self.ae = load_ae_ci(model_type, ci_name=cust_pat, device="cpu" if offload else self.device)
            else:
                self.ae = load_ae(model_type, device="cpu" if offload else self.device)
        if debug:
            self.model = load_flow_debug_model_ci(model_type, ci_name=cust_pat)
        else:
            if "fp8" in model_type:
                self.model = load_flow_model_quintized(model_type, device="cpu" if offload else self.device)
            else:
                if cust_pat != '' and 'tf' in cust_lb:
                    self.model = load_flow_model_ci(model_type, ci_name=cust_pat)
                else:
