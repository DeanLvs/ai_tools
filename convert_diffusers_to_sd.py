...
def transform_keys_with_keymap(merge_state_dict):
    """
    Transform keys in `merge_state_dict` using the `keymap` dictionary.

    Args:
    - merge_state_dict: A dictionary with keys to potentially transform.
    - keymap: A dictionary mapping old keys to new keys.

    Returns:
    - A new dictionary with transformed keys where applicable.
    """
    transformed_dict = {}
    for k, v in merge_state_dict.items():
        # Check if the current key is in the keymap
        if k in KeyMap:
            # If so, use the new key from the keymap
            new_key = KeyMap[k]
