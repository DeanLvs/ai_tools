...
def convert_ldm_unet_checkpoint(checkpoint, config={"layers_per_block":2,"num_head_channels":170}):
    """
    ...
    new_checkpoint["up_blocks.1.upsamplers.0.conv.weight"] = unet_state_dict["output_blocks.5.2.conv.weight"]
    new_checkpoint["up_blocks.1.upsamplers.0.conv.bias"] = unet_state_dict["output_blocks.5.2.conv.bias"]

    if "output_blocks.2.1.conv.weight" in unet_state_dict:
        new_checkpoint["up_blocks.0.upsamplers.0.conv.weight"] = unet_state_dict["output_blocks.2.1.conv.weight"]
    if "output_blocks.2.1.conv.bias" in unet_state_dict:
        new_checkpoint["up_blocks.0.upsamplers.0.conv.bias"] = unet_state_dict["output_blocks.2.1.conv.bias"]
...
def convert_ldm_vae_checkpoint(checkpoint, config={"layers_per_block":2,"num_head_channels":170}):
    ...
    # Apply direct key mappings first
    for old_key, new_key in direct_key_mappings.items():
        if old_key in vae_state_dict:
            value = vae_state_dict.pop(old_key)
...
if __name__ == "__main__":
    ...
    checkpoint = merge_state_dict["state_dict"]
