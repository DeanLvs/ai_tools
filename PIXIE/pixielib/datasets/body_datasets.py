...
class TestData(Dataset):
    def __init__(self, testpath, iscrop=False, crop_size=224, hd_size = 1024, scale=1.0, body_detector='rcnn', device='cpu'):
        '''
    ...
    def __getitem__(self, index):
        ...
        box_left, box_right, box_top, box_bottom = 0.0, 0.0, 0.0, 0.0
        if self.iscrop:
            ...
            if bbox is None:
                ...
                box_left, box_right, box_top, box_bottom = left, right, top, bottom
        ...
        # 保存高清图像
        hd_image_path = f'/Users/dean/PycharmProjects/Body/PIXIE/i_hd_image.jpg'
        cv2.imwrite(hd_image_path, (hd_image.transpose(1, 2, 0) * 255).astype(np.uint8))  # 转换为原始格式
        ...
        return {'image': torch.tensor(dst_image).float(),
                ...
                'bbox': bbox,
                'size': size,
                'box_index': (box_left, box_right, box_top, box_bottom),
                'tform_hd': tform_hd,
                'shp': image.shape
                }
