...
class LossToFile(TrainerCallback):
    """把训练/验证的 loss 都写到文本，便于画图
       step <tab> train_loss <tab> eval_loss <tab> lr <tab> grad_norm
    """
    def __init__(self, path="train_loss.log"):
        self.f = open(path, "a", buffering=1)          # 行缓冲
    # ✨ 统一用 on_log 就够了
    def on_log(self, args, state, control, logs=None, **kwargs):
        if not logs:
            return
        step       = state.global_step
        train_loss = logs.get("loss", float("nan"))    # 可能没有
        eval_loss  = logs.get("eval_loss", float("nan"))
        lr         = logs.get("learning_rate", 0.0)
        grad_norm  = logs.get("grad_norm", 0.0)
        self.f.write(
            f"{step}\t{train_loss:.6f}\t{eval_loss:.6f}\t"
            f"{lr:.3e}\t{grad_norm:.4f}\n"
        )
...
raw_ds   = load_dataset("json",
                        ...
                        split="train")
ds_tok   = (raw_ds.map(tok_fn, batched=True, num_proc=4,
                       remove_columns=["text"])
                    .shuffle(seed=42)
                    .with_format("torch"))          # 保持 tensor
eval_ds  = (ds_tok.select(range(10_000))           # <<< 评估集
                    .with_format("torch"))          # <<< 再次设 format
...
args = TrainingArguments(
    ...
    eval_steps= 2_000,         # <<< ②
    eval_accumulation_steps= 4              # <<< ③ 防止显存爆（可调）
)
