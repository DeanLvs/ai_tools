...
from ImageProcessorSDXCN import CustomInpaintPipeline
import base64
import numpy as np
import requests
import gc
...
def genIt(processor, prompt, prompt_2, reverse_prompt, reverse_prompt_2, genImage, big_mask,
    ...
    try:
        ...
        with torch.no_grad():
            ...
            if len(processor.controlnet_list) == 0:
                print(
                    f'run ----------------------------- use Inpaint Pipeline {prompt} {prompt_2} {reverse_prompt} {reverse_prompt_2} '
                    f'{type(prompt_embeds)} {type(pooled_prompt_embeds)} '
                    f'{type(negative_prompt_embeds)} {type(negative_pooled_prompt_embeds)} {num_inference_steps} {guidance_scale}')
                ...
                print(f'use {processor.controlnet_list}')
                for cont in processor.controlnet_list:
                    if 'key_points' == cont:
                ...
                print(
                    f'run ----------------------------- use Controlnet Pipeline {prompt} {prompt_2} {reverse_prompt} {reverse_prompt_2} '
                    f'{type(prompt_embeds)} {type(pooled_prompt_embeds)} '
                    f'{type(negative_prompt_embeds)} {type(negative_pooled_prompt_embeds)} {num_inference_steps} {guidance_scale} '
...
def handle_image_processing_b(data):
    ...
    try:
        ...
        processor = CustomInpaintPipeline(controlnet_list_t = re_p_float_array, use_out_test= (ga_b == 1.0))
        ...
        def genItOrg():
            ...
            orgResult = genIt(processor, prompt, prompt_2, reverse_prompt, reverse_prompt_2, orgImage, big_mask, num_inference_steps, guidance_scale,
                              create_callback('1/4'), strength, [control_image, normal_map_img, segmentation_image_pil, cor_map_img],  re_b_float_array,
        ...
        def genitOrgFix():
            org_fix_Result = genIt(processor, prompt, prompt_2, reverse_prompt, reverse_prompt_2, clear_org_i, big_mask, num_inference_steps, guidance_scale,
                              create_callback('2/4'), strength, [control_image, normal_map_img, segmentation_image_pil, cor_map_img],
                                   re_b_float_array,prompt_embeds, negative_prompt_embeds, pooled_prompt_embeds, negative_pooled_prompt_embeds)
        ...
        if isinstance(genImage, Image.Image) and genImage is not None:
            def genItFilledHuman():
                result = genIt(processor, prompt, prompt_2, reverse_prompt, reverse_prompt_2,
                               ...
                               create_callback('3/4'), strength, [control_image, normal_map_img, segmentation_image_pil, cor_map_img], re_b_float_array,
                               prompt_embeds, negative_prompt_embeds, pooled_prompt_embeds, negative_pooled_prompt_embeds)
