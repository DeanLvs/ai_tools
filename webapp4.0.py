...
def handle_image_processing_b(data):
    ...
    if re_mask != 'F' or not os.path.exists(filled_image_pil_path):
        ...
        mask_future, gen_fix_pic_future = re_auto_mask_with_out_head_b(filename, int(re_p), int(re_b), int(ha_p), int(ga_b), re_mask)
    ...
    try:
        ...
        if isinstance(genImage, Image.Image) and genImage is not None:
            result = genIt(processor, prompt, prompt_2, reverse_prompt, reverse_prompt_2, genImage, big_mask, num_inference_steps, guidance_scale, create_callback('3/4'), strength)
        ...
        if next_genImage:
            result_next = genIt(processor, prompt, prompt_2, reverse_prompt, reverse_prompt_2, next_genImage, big_mask, num_inference_steps, guidance_scale, create_callback('4/4'), strength)
