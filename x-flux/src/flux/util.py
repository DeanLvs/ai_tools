...
def load_flow_model_ci(name: str, ci_name='STOIQOA/STOIQOAfroditeFLUXXL_F1DAlpha.safetensors'):
    # Loading Flux
    print("Init model")
    ...
    ckpt_path = f'/nvme0n1-disk/civitai-downloader/{ci_name}'
    ...
    if ckpt_path is not None:
        ...
        for key, value in sd.items():
            # 如果键以 "model.diffusion_model." 开头，去掉这个前缀
            if key.startswith("model.diffusion_model."):
                new_key = key.replace("model.diffusion_model.", "")
                new_sd[new_key] = value.to(torch.bfloat16)
            else:
                new_sd[key] = value.to(torch.bfloat16)
        missing, unexpected = model.load_state_dict(new_sd, strict=False, assign=True)
        print_load_warning(missing, unexpected)
        model = model.to('cuda')
    return model
def load_flow_debug_model_ci(name: str, ci_name='STOIQOA/STOIQOAfroditeFLUXXL_F1DAlpha.safetensors'):
    # Loading Flux
    print("Init model")
    ckpt_path = f'/nvme0n1-disk/civitai-downloader/{ci_name}'
    with torch.device("cpu"):
...
def load_ae_ci(name: str, ci_name='STOIQOAfroditeFLUXXL_F1DAlpha.safetensors', device: str | torch.device = "cuda") -> AutoEncoder:
    ckpt_path = f'/nvme0n1-disk/civitai-downloader/{ci_name}'
    # Loading the autoencoder
    print("Init AE")
    with torch.device("meta" if ckpt_path is not None else device):
    ...
    if ckpt_path is not None:
        ...
        for key, value in sd.items():
            ...
            if key.startswith("vae."):
                ...
                new_sd[new_key] = value.to(torch.float32)
        missing, unexpected = ae.load_state_dict(new_sd, strict=False, assign=True)
        print_load_warning(missing, unexpected)
    return ae

def load_ae_debug_ci(name: str, ci_name='STOIQOAfroditeFLUXXL_F1DAlpha.safetensors', device: str | torch.device = "cuda") -> AutoEncoder:
    ckpt_path = f'/nvme0n1-disk/civitai-downloader/{ci_name}'
    # Loading the autoencoder
    print("Init AE")
    with torch.device("meta" if ckpt_path is not None else device):
