...
def re_auto_mask_with_out_head_b(filename, re_p=30, re_b=0, ha_p=30, ga_b=0, re_mask = 'F', extracted_textures = None):
    ...
    def process_fix_pic_mask():
        ...
        clear_result, filled_image_pil, mask_clear_finally= gen_fix_pic(file_path, image_pil, mask_clear, not_close_mask_img, extracted_textures)
    ...
    if human_mask_ndarray:
        merged_mask = subtract_masks(mask_clear_fin, human_mask_ndarray)
    else:
        merged_mask = np.array(mask_clear_fin)
    if re_p > 0:
...
def handle_image_processing_b(data):
    ...
    try:
        ...
        emit('processing_done_text', {'text': '正在book yes(不可言说，再次点击获取历史结果)'}, broadcast=True,
             to=room_id)
    finally:
        # 释放 GPU 内存
        del result
