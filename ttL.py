...
class ImageProcessor(metaclass=SingletonMeta):
    def __init__(self, model_path="stable-diffusion-inpainting", checkpoint_paths=None, lora_paths = None):
        ...
        for idx, checkpoint_path in enumerate(self.checkpoint_paths):
            try:
                ...
                print(ex)
                continue
        for idx, lora_path in enumerate(self.lora_paths):
            print("load lora")
            try:
                if idx == 0:
                    self.load_lora_weights(checkpoint_path)
                else:
                    self.load_lora_weights(checkpoint_path, True)
            except Exception as ex:
                print(ex)
                continue
    ...
    def load_lora_weights(self, lora_path, lor_h = False):
        # 确保此路径和文件名正确
        if not os.path.exists(lora_path):
            raise FileNotFoundError(f"File not found: {lora_path}")
...
if __name__ == "__main__":
    ...
    processor = ImageProcessor(checkpoint_paths= [checkpoint_path], lora_paths= [lora_path])
