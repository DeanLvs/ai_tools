...
# 本地 stable-diffusion-inpainting 模型路径
local_model_path = "stable-diffusion-inpainting"  # 替换为你的本地路径
# 加载本地 stable-diffusion-inpainting 模型
pipe = StableDiffusionInpaintPipeline.from_pretrained(local_model_path)
pipe.to(device)

# 加载 V8-inpainting.safetensors 模型
model_path = "/mnt/sessd/civitai-downloader/pornmasterPro_v8-inpainting.safetensors"  # 本地 Checkpoint Merge 模型路径
model_state_dict = load_file(model_path)

# 将 V8-inpainting.safetensors 模型权重加载到 UNet 模型中
